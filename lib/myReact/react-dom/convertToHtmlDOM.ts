import { isReactDOMSVGElement, isReactDOMTextNode } from '@myReact/utils/typeGuards';

export default function convertToHtmlDOM(node: ReactDOM.Node): Element | Text {
  ...
  element.children.map(child => convertToHtmlDOM(child)).forEach(child => dom.appendChild(child));
